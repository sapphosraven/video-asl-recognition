...
def convert_everything_to_mp4():
    raw_dir = r'F:\Uni_Stuff\6th_Sem\DL\Proj\video-asl-recognition\pose_estimation\data\raw_videos_mp4'
    for fname in os.listdir(raw_dir):
        base, ext = os.path.splitext(fname)
        if ext.lower() in ['.webm', '.mkv']:
            src = os.path.join(raw_dir, fname)
            dst = os.path.join(raw_dir, base + '.mp4')
            if not os.path.exists(dst):
                cmd = f'ffmpeg -y -i "{src}" "{dst}"'
                print(f"Converting {fname} to {base + '.mp4'}")
                os.system(cmd)
...
def video_to_frames(video_path, size=(256, 256)):
    ...
    size -> (int, int), width, height. Default is (256, 256) for MediaPipe compatibility.
    """
    print(f"[DEBUG] Extracting frames from {video_path} with size {size}")
...
def convert_frames_to_video(frame_array, path_out, size, fps=25):
    print(f"[DEBUG] Saving {len(frame_array)} frames to {path_out} with size {size}")
    ...
    out.release()
    print(f"[DEBUG] Saved video to {path_out}")


def extract_frame_as_video(src_video_path, start_frame, end_frame):
    print(f"[DEBUG] Extracting frames {start_frame} to {end_frame} from {src_video_path}")
...
def main():
    ...
    content = json.load(open(r"F:\Uni_Stuff\6th_Sem\DL\Proj\video-asl-recognition\pose_estimation\data\WLASL100.json"))
